  ...
  return (
    <nav className="sticky top-0 z-50 card-dark border-b border-border backdrop-blur-sm">
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-4 px-4 py-3">
        <Link to="/home" className="gradient-text text-xl font-bold tracking-tight">MiniSocial</Link>
              ...
              >
                {results.map((u) => (
                  <div key={u._id} className="flex items-center gap-3 px-4 py-3 hover:bg-secondary/50 transition-colors">
                    ...
                    <Link to={`/profile/${u._id}`} onClick={() => setShowResults(false)} className="flex-1 text-sm font-medium text-card-foreground hover:underline">
                      {u.name}
                    </Link>
                    {u._id !== user?._id && (
                      <motion.button
                        ...
                        className={`rounded-md px-3 py-1 text-xs font-medium transition ${sentRequests[u._id] ? 'bg-muted text-muted-foreground' : 'gradient-bg text-primary-foreground hover:opacity-90'}`}
                      >
        ...
        <div className="flex items-center gap-1">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate('/messages')} className="rounded-lg p-2 text-muted-foreground hover:bg-secondary hover:text-card-foreground transition">
            <MessageCircle className="h-5 w-5" />
          </motion.button>
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => navigate('/profile/me')} className="rounded-lg p-2 text-muted-foreground hover:bg-secondary hover:text-card-foreground transition">
            <User className="h-5 w-5" />
          </motion.button>
          <motion.button whileTap={{ scale: 0.9 }} onClick={handleLogout} className="rounded-lg p-2 text-muted-foreground hover:bg-secondary hover:text-card-foreground transition">
